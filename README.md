...
- 라운드 끝날 때 진출자 수가 **16/8/4/2/1명**이면 자동으로 폴더에 백업됨
  - 8강 → `public/round-8/`
  - 4강 → `public/round-4/`
  - 결승 진출자 2명 → `public/round-2/`
  - 우승작 → `public/champion/`
  - 16강은 시작 사진이 32장 이상이라 한 라운드 후 16명이 남을 때만 생성
  - 시작 사진 수에 따라 일부 단계가 건너뛰어질 수 있음 (예: 20장으로 시작 → 10/5/3/2/1로 줄어들면 round-8/4/16 폴더는 안 생김)
